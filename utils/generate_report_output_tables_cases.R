...
source('utils/spim_table_cases.R')

print("Summarising")

evaluation_summary <- filter(total_output_table, date==ymd(evaluation_date)) 

population <- read_csv("data/population.csv")

evaluation_summary <- left_join(evaluation_summary, population, by="Country") %>%
  mutate(R=paste0(round(`50%_RT`,2), " (", round(`5%_RT`,2), ", ", round(`95%_RT`,2), ")"),
         incidence=paste0(round(`50%_inf`/population,0), " (", round(`5%_inf`/population,0), ", ", round(`95%_inf`/population,0), ")") ,
         growth=paste0(percent(`50%_growth`, .01), " (", percent(`5%_growth`, .01), ", ", percent(`95%_growth`, .01), ")")) %>%
  select(date, Country, R, incidence, growth)

write_csv(evaluation_summary, paste0(results_fp, JOBID, "_summary.csv"))
